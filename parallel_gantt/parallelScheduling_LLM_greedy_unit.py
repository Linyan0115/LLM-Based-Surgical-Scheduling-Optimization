...
def LLM_strategy(num_workshops,df):
    # 第二部分：调用LLM来获取优化策略
    # 提取数据，格式化成 LLM 需要的格式
    jobs_data = df.apply(lambda row: f"[{row['产品']}, {row['到达时间']}, {row['生产天数']}]",
                         axis=1).tolist()
        ...
        messages=[
                {
                    ...
                    "content": f"你是一个并行机车间优化专家，data中给出了一个并行机生产车间的相关信息。"
                               ...
                               f"每个工件可以选择任一机器上被加工，目标是最短时间完成所有工件。车间数量为：{num_workshops}。"
                               f"数据如下：{data_string}。"
                               f"请确保输出包含所有工件在各机器的具体分配方案，且每个产品的分配方案在列表中仅出现一次。同一时间一个车间只能加工一件产品，目标是最短时间完成所有工件"
...
def main():
    ...
    # 读取Excel文件
    df = pd.read_excel("product_schedule_unit.xlsx")
