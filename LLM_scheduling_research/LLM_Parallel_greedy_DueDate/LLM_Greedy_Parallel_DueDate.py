...
def LLM_strategy(num_workshops, df):
    # 第二部分：调用LLM来获取优化策略
    jobs_data = df.apply(lambda row: f"[{row['Product']}, {row['Product Type']}, {row['Arrival Time']}, {row['Production Days']}, {row['Deadline']}]",
                         axis=1).tolist()
        ...
        messages=[
                {
                    ...
                    "content": f"你是一个并行机车间优化专家，data中给出了一个并行机生产车间的相关信息。"
                               f"数据集格式为：产品ID, 分别表示工件名称, 到达时间, 生产天数，产品到期时间。"
                               f"每个工件可以选择任一机器上被加工，目标是尽快完成所有工件，最后的结束时间一定要尽可能最短，且要求所有工件必须在到期时间之前完成。"
                               f"车间数量为：{num_workshops}。数据如下：{data_string}。"
                               f"请确保输出包含所有工件在各机器的具体分配方案，且每个产品的分配方案在列表中仅出现一次。"
                               f"如果出现超期的情况，请跳过该产品，并单独列举说明。"
                               f"以及每个工件在机器上的开始时间和结束时间。请严格按照以下格式输出json列表，不要包含其他内容，不需要给出任何中间解答过程，直接给出json格式的答案，一定不要有其他任何信息："
                               f"[Product，workshop, 开始生产时间, 结束生产时间]"
                               f"最后一定要计算总完成时间"
                               f"过期产品跳过生产并额外标出"
                },
            ]
...
def greedy(df, num_workshops, workshop_free_time, schedule_result):
    # 第一部分：贪心算法分配产品到最早空闲的车间
    for index, row in df.iterrows():
        product_id = row['Product']
        arrival_time = row['Arrival Time']
        processing_days = row['Production Days']
        ...
        end_time = start_time + processing_days

        # 检查是否可以在到期时间之前完成
        if end_time > due_date:
            print(f"警告: 产品 {product_id} 无法在到期时间 {due_date} 之前完成，跳过该产品或调整调度策略")
            continue  # 跳过这个产品，或者可以在这里实现更多复杂的逻辑来重新分配
...
def judge(schedule_result, llm_result_json):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"存在一个问题：有10个一样的车间，想把我这些产品分配到不同的车间，但希望车间效率利用到最高（生产完产品的总时间要尽量小），下面是贪心和llm得出的答案:{final_result_str},请你帮我比较之后得出llm更先进的结论"
            },
        ]
    )
    print(response.choices[0].message.content)
...
def main():
    # 读取Excel文件
    df = pd.read_csv("product1-50.csv")

    # 初始化三个车间的可用时间
    num_workshops = 10
    workshop_free_time = [1] * num_workshops
    ...
    # 调用贪心算法
    greedy(df, num_workshops, workshop_free_time, schedule_result)

    # 调用 LLM 策略
    llm_result_json = LLM_strategy(num_workshops, df)

    # 进行结果比较
    judge(schedule_result, llm_result_json)
