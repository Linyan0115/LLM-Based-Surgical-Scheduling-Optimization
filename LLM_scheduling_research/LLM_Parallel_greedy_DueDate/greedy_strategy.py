...
def LLM_strategy(num_workshops, df):
    # 第二部分：调用LLM来获取优化策略
    jobs_data = df.apply(lambda row: f"[{row['Product']}, {row['Arrival Time']}, {row['Production Days']}, {row['Deadline']}]",
                         axis=1).tolist()
        ...
        messages=[
                {
                    ...
                    "content": f"你是一个并行机车间优化专家，data中给出了一个并行机生产车间的相关信息。"
                               ...
                               f"每个工件可以选择任一机器上被加工，目标是尽快完成所有工件，且要求所有工件必须在到期时间之前完成。"
...
def main():
    # 读取Excel文件
    df = pd.read_csv("product_production_500.csv")
